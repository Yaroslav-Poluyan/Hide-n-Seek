    ...
    public class DefaultSceneLoaderWindow : EditorWindow
    {
        ...
        private void OnGUI()
        {
            ...
            foreach (var scene in ScenesInBuild)
            {
                ...
                if (sceneName.Contains("Arena") || sceneName.Contains("Map"))
                {
                    if (GUILayout.Button("SetToLoad"))
                    {
                        if(!isThisSceneSetToLoad) EditorPrefs.SetString("SkipStatesArena", sceneName);
                        else EditorPrefs.SetString("SkipStatesArena", "");
                    }
                }
